...
namespace _2._05.FootballTeamGenerator.Models
{
    public class Stats
    {
        ...
        => (this.Endurance + this.Sprint + this.Dribble + this.Passing + this.Shooting) / STATS_CNT;

        private void ValidateStat(string name, int value)
        {
            if (value < MIN_STAT || value > MAX_STAT)
            {
                throw new ArgumentException(String.Format(GlobalConstants.INVALID_STAT_EXC_MSG, name));
            }
        }
